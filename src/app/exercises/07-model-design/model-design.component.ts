...
import { form, FormField, required, email, disabled } from '@angular/forms/signals';
      ...
      <section class="exercise-section">
        <details class="hint-details">
          <summary class="hint-summary">
            ...
            <span>domainToForm / formToDomain pattern (click to expand)</span>
          </summary>
          <div class="hint-content">
            <pre class="code-block">// Domain → Form: flatten nested objects
function domainToForm(user: User): UserFormModel &#123;
  return &#123;
...
// Form → Domain: re-nest flat fields
function formToDomain(form: UserFormModel): Partial&lt;User&gt; &#123;
  return &#123;
...
&#125;</pre>
      ...
      <section class="exercise-section">
        <details class="hint-details">
          <summary class="hint-summary">
            <span class="hint-icon">💡</span>
            <span>linkedSignal + rxResource pattern (click to expand)</span>
          </summary>
          <div class="hint-content">
            <pre class="code-block">// Load from API
userResource = rxResource(&#123;
  stream: () =&gt; this.api.getUser('user-1')
&#125;);

// Derive form model from resource
...
disabled(f, () =&gt; this.userResource.isLoading());</pre>
          </div>
        </details>
      </section>
    </div>
...
export class ModelDesignComponent {
  ...
  // TODO 3: Use rxResource to load user from API
  // userResource = rxResource({
  //   stream: () => this.api.getUser('user-1')
  // });
  ...
  // TODO 4: Use linkedSignal to derive form model from API resource
  // When userResource.value() changes, re-derive the form model
  ...
  protected readonly profileForm = form(this.formModel, (f) => {
    // TODO 5a: Add validators
    // required(f.displayName);
    // required(f.email);
    // email(f.email);

    // TODO 5b: Disable form while resource is loading
