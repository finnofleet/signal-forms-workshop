...
export class BasicsComponent {
  // 1. Form Model (with BONUS fields)
  ...
  // 2. Form with validation schema (with BONUS validations)
  protected readonly loginForm = form(this.loginModel, (f) => {
    // BONUS: Username validation
    required(f.username);
    ...
    // Password validation (with BONUS minLength)
    required(f.password);
    minLength(f.password, 6);

    // rememberMe: no validation needed
  });

  // Debug info (with BONUS fields)
  protected formDebugInfo = () => {
    return JSON.stringify({
      ...
      username: {
        value: this.loginForm.username().value(),
        valid: this.loginForm.username().valid(),
        touched: this.loginForm.username().touched(),
        errors: this.loginForm.username().errors()
      },
      email: {
      ...
      rememberMe: {
        value: this.loginForm.rememberMe().value()
      }
    }, null, 2);
  };
